...
1. **Headless Go Backend**:
   ...
   - 20-byte binary UDP wire protocol with in-band VAD (<30ms speaking indicator propagation) and 10-20ms Opus frame forwarding.
   - WebSocket JSON-RPC control plane for real-time text chat, presence roster, channel tree, role-based authorization, and moderation.
...
2. **Native Flutter Desktop Client (Windows & Linux)**:
   ...
   - Clean Architecture state management (Riverpod) handling WebSocket control events, presence sync, and voice states.
   - Native audio engine (`libvoice_engine` / `miniaudio` + `libopus` via Dart FFI / Audio Worker) with WASAPI / PipeWire backends, hardware device enumeration, and PTT / VAD metering.
...
| F16 | In-Band Fast VAD | Embedded VAD bit and energy level in UDP header for <30ms speaking sync | M2, M3 | R2 |
...
| F26 | Member Kick Action | Immediate WebSocket disconnection (code 4001) and UDP session revocation | M4 | R3 |
| F27 | Real-Time State Sync Broadcast | Instant WebSocket JSON-RPC event broadcast for all channel/user state changes | M1, M4 | R3 |
...
- `Payload Length`: 16-bit uint byte count of payload
...
- **Kick Member**: `{"action": "kick_member", "target_user_id": 2, "reason": "Rule violation"}` -> WebSocket close code `4001` on target client + broadcast `{"event": "member_kicked", "user_id": 2}`
...
│   │   ├── services/             # WebSocket client, UDP voice client, PTT/VAD
│   │   ├── state/                # Riverpod providers & state notifiers
│   │   └── ui/                   # 3-Pane Desktop Layout
│   │       ├── channels/         # Left: Server tree & channel lists
│   │       ├── chat/             # Center: Text chat stream & message input
│   │       ├── voice/            # Center: Active voice HUD & controls
│   │       ├── roster/           # Right: Live member presence roster
│   │       └── dialogs/          # Settings, Audio Device selection, Admin mod
│   ├── native/                   # C Native Audio Engine (libvoice_engine)
│   │   ├── src/
│   │   ├── CMakeLists.txt
│   │   └── include/
│   ├── pubspec.yaml
...
└── test/                         # E2E Test Suite & Synthetic Load Harness
    ...
    ├── tier1_features/           # Feature coverage tests
    ├── tier2_boundaries/         # Boundary and corner tests
    ├── tier3_interactions/       # Cross-feature interaction tests
